...
const itemText = item => { const paid = item.paidSubtotalCent === undefined ? item.subtotalCent : item.paidSubtotalCent; return { id: item._id || item.skuId, skuId: item.skuId || '', name: item.productNameSnapshot || '订单商品', spec: [item.specSnapshot, item.packageUnitSnapshot].filter(Boolean).join(' · ') || '规格以订单快照为准', quantity: Number(item.quantity || 0), media: item.mediaSnapshot || '', unitPrice: Number.isInteger(Number(item.unitPriceCent)) ? money(Number(item.unitPriceCent) / 100) : '', subtotal: Number.isInteger(Number(paid)) ? money(Number(paid) / 100) : '' }; };

function viewModel(order, items, refund) {
  ...
  return {
    id: order._id, orderNo: order.orderNo || order._id, rawStatus: order.status, statusText: STATUS_LABELS[order.status] || order.status,
    ...
    fulfillmentType, isPickup: fulfillmentType === 'pickup', address: order.addressSnapshot || {}, pickupSite: order.pickupSiteSnapshot || {}, deliverySlot: order.deliverySlotSnapshot || {},
    ...
    canRefund: order.paymentStatus === 'paid' && ['pending_confirmation', 'picking', 'shipping', 'delivered', 'completed'].includes(order.status) && !activeAfterSale && order.refundStatus !== 'refunded', canReview: order.status === 'completed', canInvoice: order.status === 'completed' && order.paymentStatus === 'paid'
  };
}
...
Page({
  ...
  async loadOrder() {
    ...
    this.setData({ status: 'ready', order: viewModel(rawOrder, items, refund), errorText: '' });
  },
  ...
  async preparePayment() {
    ...
    try {
      ...
      if (!params) return wx.showToast({ title: result && result.error && result.error.message || '真实微信支付参数尚未配置', icon: 'none' });
  ...
  async repurchase() { const me = await auth.getMe(); const user = me && me.ok && me.data && me.data.user; if (!user || user.userType !== 'b' || user.businessStatus !== 'approved') return wx.showToast({ title: '再次购买仅对企业采购账户开放', icon: 'none' }); wx.navigateTo({ url: `/package-business/pages/repurchase/index?orderId=${encodeURIComponent(this.data.orderId)}` }); },
