...
Page({
  data: { order: null, orderId: '', loading: true, failed: false, retryable: false, errorText: '订单读取失败，请重试或返回订单页核对。' },
  async onLoad(query) {
    const id = query && typeof query.id === 'string' ? query.id.trim() : '';
    if (!id) return this.setData({ order: null, orderId: '', loading: false, failed: true, retryable: false, errorText: '缺少订单编号，请返回订单页核对。' });
  ...
  async loadOrder(id) {
    ...
    if (!rawOrder) {
      ...
      return this.setData({ order: null, loading: false, failed: true, retryable, errorText: result && result.error && result.error.message || (retryable ? '订单读取失败，请重试或返回订单页核对。' : '暂时无法查看此订单，请返回订单页核对。') });
    }
    const items = result.data.items || rawOrder.items || rawOrder.itemsSnapshot || [];
    this.setData({ order: toOrderRow({ ...rawOrder, items }), loading: false, failed: false, retryable: false, errorText: '' });
  },
