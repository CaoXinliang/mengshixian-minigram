...
Page({
  ...
  async load() {
    this.setData({ status: 'loading', errorText: '', intent: null });
    const me = await auth.getMe();
    if (!me || !me.ok || !me.data || !me.data.user) return this.setData({ status: 'forbidden', errorText: '请先登录后查看储值账户' });
    ...
    this.setData({ status: 'ready', account: { balance: money(source.balanceCent), frozen: money(source.frozenAmountCent), enabled: source.enabled === true, topupEnabled: source.topupEnabled === true }, ledger: rows.map(row => ({ ...row, amountText: `${Number(row.amountCent || 0) > 0 ? '+' : ''}${money(row.amountCent)}`, label: row.description || row.type || '账户变动', time: dateText(row.createdAt) })) });
  },
