...
const { createReviewSubmission, reviewStatusText } = require('../../../modules/review-submission');
const MAX_MEDIA = 6;
...
Page({
  ...
  async load() {
    ...
    const mine = (mineResult.data && mineResult.data.rows || []).map(item => ({ ...item, statusText: reviewStatusText(item.status) })); const eligible = eligibleResult.data && eligibleResult.data.rows || [];
  ...
  async selectOrderById(id) { const items = (this._eligibleItems || []).filter(item => item.orderId === id); if (!items.length) return wx.showToast({ title: '该订单暂无可评价商品', icon: 'none' }); this._reviewSubmission = createReviewSubmission({ createReview: payload => reviews.create(payload) }); this.setData({ current: { id, orderLabel: orderLabel(items[0].orderNo, id), items: items.map(item => ({ id: item.orderItemId, skuId: item.skuId, name: item.productNameSnapshot || '订单商品', selected: true, submissionStatus: 'pending', submissionError: '' })) }, rating: 5, content: '', media: [] }); },
  ...
  async submit() { if (this.data.busy || !this.data.current) return; const items = this.data.current.items.filter(item => item.selected); if (!items.length) return wx.showToast({ title: '请选择评价商品', icon: 'none' }); if (!this.data.content.trim()) return wx.showToast({ title: '请填写评价内容', icon: 'none' }); this.setData({ busy: true }); try { for (const item of this.data.media) if (item.status !== 'uploaded') await this.upload(item.id); const failed = this.data.media.filter(item => item.status !== 'uploaded'); if (failed.length) return wx.showToast({ title: '有图片上传失败，请重试', icon: 'none' }); if (!this._reviewSubmission) this._reviewSubmission = createReviewSubmission({ createReview: payload => reviews.create(payload) }); const result = await this._reviewSubmission.submit({ orderId: this.data.current.id, items, rating: this.data.rating, content: this.data.content.trim(), mediaIds: this.data.media.map(media => media.mediaId) }); const stateById = Object.fromEntries(result.items.map(item => [item.id, item])); this.setData({ 'current.items': this.data.current.items.map(item => stateById[item.id] ? { ...item, submissionStatus: stateById[item.id].submissionStatus, submissionError: stateById[item.id].submissionError } : item) }); if (result.kind !== 'completed') return wx.showToast({ title: result.text, icon: 'none' }); this.setData({ current: null }); await this.load(); wx.showToast({ title: '评价已提交，等待审核', icon: 'success' }); } finally { this.setData({ busy: false }); } },
