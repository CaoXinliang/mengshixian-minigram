...
Page({
  ...
  async load() {
    ...
    if (!me || !me.ok || !me.data || !me.data.user) return this.setData({ status: 'forbidden', errorText: '请先登录后管理电子发票' });
    ...
    this.setData({ status: 'ready', titles, records: records.map(item => ({ ...item, amountText: money(item.amountCent || item.invoiceAmountCent), titleText: item.titleSnapshot && item.titleSnapshot.name || item.titleName || '电子发票', statusText: item.statusText || item.status || '处理中' })), orders: availableOrders });
  },
  retry() { return this.load(); },
  showForm(event) {
    const id = event.currentTarget.dataset.id || '';
    const source = this.data.titles.find(item => item._id === id);
    this.setData({ formVisible: true, form: source ? { id: source._id, type: source.type || 'personal', name: source.name || '', taxNo: source.taxNo || '', address: source.address || '', bankName: source.bankName || '', bankAccount: source.bankAccount || '' } : { id: '', type: 'personal', name: '', taxNo: '', address: '', bankName: '', bankAccount: '' } });
  },
