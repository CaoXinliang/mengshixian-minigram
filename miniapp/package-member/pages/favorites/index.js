...
Page({
  ...
  async load() {
    ...
    this.setData({ status: rows.length ? 'ready' : 'empty', rows: rows.map(row => { let product = {}; let sku = {}; for (const item of products) { const found = (item.skus || []).find(entry => entry._id === row.skuId); if (found) { product = item; sku = found; break; } } return { id: row._id, productId: product._id || '', skuId: row.skuId, name: product.name || '收藏商品', spec: sku.specName || sku.packageUnit || '暂无规格信息', image: product.coverUrl || product.image || '/assets/products/placeholder.svg', unavailable: !product._id }; }) });
  },
