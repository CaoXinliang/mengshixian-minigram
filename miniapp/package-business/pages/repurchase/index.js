...
Page({
  ...
  onLoad(query = {}) { let orderId = query.orderId || ''; try { orderId = decodeURIComponent(orderId); } catch (error) { orderId = ''; } this.setData({ orderId }); },
  onShow() { return this.load(); },
  async load() {
    const access = await authorizeBusinessPage(this, auth, { forbiddenText: '再次购买仅对已审核企业采购账户开放', clear: { rows: [], invalidItems: [], commitLocked: false, commitError: '', addedItems: [] }, reset: () => { this._key = ''; } });
    if (!access) return;
    if (!this.data.orderId) return this.setData({ status: 'empty', errorText: '原订单编号无效', rows: [], invalidItems: [], addedItems: [] });
    ...
    const describe = item => { const snapshot = snapshotBySku.get(item.skuId) || {}; return { ...item, name: item.productNameSnapshot || item.productName || item.name || snapshot.productNameSnapshot || '订单商品', spec: item.specSnapshot || item.specName || snapshot.specSnapshot || snapshot.packageUnitSnapshot || '' }; };
