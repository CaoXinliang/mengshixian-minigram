...
const couponValueText = source => {
  if (Number(source.discountCent) > 0) return `¥${money(source.discountCent)}`;
  const reductionBps = Number(source.discountRateBps);
  // The API stores the reduction rate, not the proportion the customer pays.
  return Number.isFinite(reductionBps) && reductionBps > 0 && reductionBps <= 10000 ? `减${reductionBps / 100}%` : '优惠';
};
...
Page({
  ...
  async load() {
    ...
    const normalize = item => { const source = item.snapshot ? { ...item.snapshot, ...item } : item; return { ...source, id: item._id, name: source.name || source.title || '优惠券', valueText: couponValueText(source), thresholdText: Number(source.minSpendCent || 0) > 0 ? `满 ¥${money(source.minSpendCent)} 可用` : '无门槛', validityText: [source.validFrom, source.validTo].filter(Boolean).map(v => String(v).slice(0, 10)).join(' 至 '), statusText: source.statusText || ({ active: '可领取', available: '可使用', used: '已使用', expired: '已过期' }[item.status] || '暂不可使用'), usable: item.status === 'available' }; };
