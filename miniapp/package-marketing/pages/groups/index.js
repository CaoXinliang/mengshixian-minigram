...
Page({
  ...
  async load() {
    ...
    const campaigns = (campaignResult.data && campaignResult.data.rows || []).map(item => ({ ...item, id: item._id, name: item.name || item.title || '限时拼团', groupSize: Number(item.groupSize || item.targetCount || 0), endText: timeText(item.endAt), statusText: item.statusText || ({ active: '进行中', ended: '已结束', paused: '已暂停' }[item.status] || '活动状态待确认') }));
    const mine = (mineResult.data && mineResult.data.rows || []).map(entry => { const item = entry.group || entry; return { ...item, member: entry.member || null, id: item._id, name: item.groupNo || '我的拼团', joined: Number(item.memberCount || item.reservedMemberCount || 0), target: Number(item.groupSize || 0), statusText: ({ open: '待成团', success: '已成团', failed: '未成团' }[item.status] || '处理中'), endText: timeText(item.expiresAt) }; });
