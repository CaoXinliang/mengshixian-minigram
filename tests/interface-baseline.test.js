...
assert(js.includes('function orderCategoriesForHome(rows)') && js.includes('const remoteCategories = orderCategoriesForHome(categoryRows);'), 'remote category data must use the approved shared presentation order rather than incidental database order');
